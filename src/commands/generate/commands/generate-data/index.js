...
const bar = new cliProgress.SingleBar(
	{
		format:
			'CLI Progress |' +
			colors.cyan('{bar}') +
			'| {percentage}% || {value}/{total} attributes will be generated by AI ',
		barCompleteChar: '\u2588',
		barIncompleteChar: '\u2591',
		hideCursor: true
	},
...
export default class GenerateDataCommand {
	...
	exec(schemaNameList, parsedObject, openApiKey) {
		...
		for (let i = 0; i < schemaNameList.length; i++) {
			...
			if (context.ignoreList.indexOf(targetSchemaName) !== -1) {
				Logger.warn(
					`[ignore] ${context.outputPath}/${targetSchemaName}${context.extension}`
				);
				continue;
			}
	...
	replaceStringPropertyDataByOpenAI = async (apiKey, numberOfData) => {
		...
		Logger.syslog(
			`Test data for attributes [${Array.from(propertySet).join(
				','
			)}] are generated by AI`
		);
		bar.start(propertySet.size, 0); // progress bar
		let progress = 0;
